...
def generate_cap_for_package_aid(aid: bytes, major: int, minor: int, identifier: int, package_name: str) -> str:
    imported_packages = []
    ...
    f = open(path.join(BASE_PATH, 'template', 'applets', 'javacard', 'Import.cap'), 'wb')
    f.write(bytes.fromhex(import_section))
    f.close()

    # create new cap file by zip of directories
    shutil.make_archive(f'test_{package_name}_{identifier}.cap', 'zip', path.join(BASE_PATH, 'template'))

    # remove zip suffix
    if os.path.exists(f'test_{package_name}_{identifier}.cap'):
        os.remove(f'test_{package_name}_{identifier}.cap')
    os.rename(f'test_{package_name}_{identifier}.cap.zip', f'test_{package_name}_{identifier}.cap')

    return f'test_{package_name}_{identifier}.cap'