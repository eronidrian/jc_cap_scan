...
class AIDScanner:
    ...
    def test_aid(self, tested_package_aid, supported_list, tested_list, classes_supported_list, class_range):
        imported_packages = []
        ...
        if self.check_aid(import_content, tested_package_aid, self.is_installed):
            ...
            self.is_installed = self.check_classes_range(import_content, tested_package_aid, self.is_installed,
                                                         classes_supported_list, class_range)
        else:
    ...
    def run_scan_recursive(self, modified_ranges_list, package_aid, major, minor, supported, tested,
        ...
        for value in range(start, stop + 1):
            ...
            if len(local_modified_ranges_list) == 0:
                ...
                self.test_aid(new_package, supported, tested, classes_supported_list, class_range)
                self.num_tests += 1
            else:
    ...
    def run_scan(self, cfg, supported, tested, classes_supported_list, class_range):
        print("################# BEGIN ###########################\n")
        print(cfg)
        print("###################################################\n")
    ...
    def scan_jc_api_305(self, card_info, supported, tested, classes_supported, class_range, supported_caps):
        MAX_MAJOR = 1
        ADDITIONAL_MINOR = 1
        ...
        self.run_scan(TestCfg("A0000000620001", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620002", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620003", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A0000000620101", 1, MAX_MAJOR, 0, 6 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A000000062010101", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620102", 1, MAX_MAJOR, 0, 6 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A0000000620201", 1, MAX_MAJOR, 0, 6 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620202", 1, MAX_MAJOR, 0, 3 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A0000000620203", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620204", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A0000000620205", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A000000062020801", 1, MAX_MAJOR, 0, 1 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A00000006202080101", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A000000062020802", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A000000062020803", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A000000062020804", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A0000000620209", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
        self.run_scan(TestCfg("A000000062020901", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.print_supported(supported, supported_caps)

    def scan_globalplatform_api(self, card_info, supported_caps, supported, tested, classes_supported, class_range):
        MAX_MAJOR = 1
        ADDITIONAL_MINOR = 1
        # minor is tested with ADDITIONAL_MINOR additional values higher than expected from the given version of JC SDK).
        # If highest version is detected, additional inspection is necessary - suspicious (some cards ignore minor version)
        ...
        # intermediate results are saved after every tested package to preserve info even in case of card error
        self.run_scan(TestCfg("A00000015100", 1, MAX_MAJOR, 0, 8 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015102", 1, MAX_MAJOR, 0, 4 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015103", 1, MAX_MAJOR, 0, 1 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015104", 1, MAX_MAJOR, 0, 1 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015105", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015106", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A00000015107", 1, MAX_MAJOR, 0, 0 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)

        self.run_scan(TestCfg("A0000000030000", 1, MAX_MAJOR, 0, 1 + ADDITIONAL_MINOR), supported, tested, classes_supported, class_range)
        self.save_scan(card_info, supported_caps, supported, tested, classes_supported)
    ...
    def save_scan(self, card_info, supported_cap_versions, supported, tested, classes_supported):
    ...
    def scan_jc_api_305_complete(self, class_range):
        ...
        for min_class_range, max_class_range in class_range:
            ...
            if max_class_range < min_class_range:
                print("Min Class Range should be smaller Max Class Range")
                return
        ...
        #tested = {}
        elapsed = -time.perf_counter()
        self.scan_jc_api_305(card_info, supported, tested, classes_supported, class_range, supported_caps)
        elapsed += time.perf_counter()
