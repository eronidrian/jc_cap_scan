...
def field_bruteforce(result_file: str, field_token_range: tuple[int, int], base_aid: bytearray, base_major: int, base_minor: int, base_class_token: int, field_type: Literal['static', 'virtual'],
    ...
    for field_token in range(field_token_range[0], field_token_range[1]):
        ...
        cap_file.import_component.packages[1] = PackageInfo(cap_file, base_minor, base_major, base_aid)
        ...
        print(f"Field token: {field_token}\n"
              f"Response: {response}\n"
              f"Success: {success}")
...
def main(argv: list[str]):
    ...
    parser.add_argument("--class_token", help="Class token to use for the base package", type=int, required=True)
